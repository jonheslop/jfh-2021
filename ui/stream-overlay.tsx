  ...
  return (
    <>
      ...
      <div className="fixed inset-4 lg:inset-16 flex items-center justify-center">
        {prevPhoto && 
        <Link href={`/stream/${prevPhoto}`} className="self-stretch flex items-center text-6xl text-white p-4 flex-1 hover:-translate-x-8 transition-all">&larr;</Link>}
        {!prevPhoto &&
        <span className="self-stretch flex items-center justify-end flex-1"/>}
        
        <div className={`bg-white p-2 lg:p-6 2xl:p-8 grid ${isExpanded ? "grid-cols-3" : "grid-cols-1"} gap-2 lg:gap-4 relative w-100 group ${nextPhoto === null ? "mr-auto" : "mr-auto"}`}>
          <img
        ...
        {nextPhoto &&
        <Link href={`/stream/${nextPhoto}`} className="self-stretch flex items-center justify-end text-6xl text-white p-4 flex-1 hover:translate-x-8 transition-all">&rarr;</Link>}
        {!nextPhoto &&
        <span className="self-stretch flex items-center justify-end flex-1"/>}
      </div>
    </>

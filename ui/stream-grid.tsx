...
const StreamGrid = ({
  ...
  const gridClasses = currentWeekOnly
    ? `grid-cols-2  ${photos.length > 4 ? 'md:grid-cols-8' : 'md:grid-cols-4'}`
    ...
    <div className={`${classes} grid gap-16 bg-white dark:bg-black`} {...props}>
      {grouped.map(({ week, weekBegins, posts }) => {
        return (
          <div key={week}>
            {!currentWeekOnly && (
              <Heading classes="md:sticky top-24 mb-8 mix-blend-difference text-white">
                <abbr
